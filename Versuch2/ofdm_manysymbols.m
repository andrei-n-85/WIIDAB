...
% SENDER
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
L = 10; % Number of OFDM symbols
N = 1536; % Number of subcarriers (without DC); N has to be larger than 1
T_u = 2048; % Symbol duration without Guardinterval in samples (1. DAB-TM: T_u = 1e-3 s = 2048 samples); 1/T_u is equal to subcarrierspacing; T_u has to be larger than N
CP_duration = 504; % Duration of Cyclic Prefix in samples; CP_duration has to be smaller than T_u
...
scale = sqrt(mean(abs(s_TP).^2));
PAPR = 10*log10(max(s_TP.*conj(s_TP))/mean(s_TP.*conj(s_TP)));
disp(PAPR);
...
for ii = 0 : L-1
    ...
    % OFDM-Demodulation
    r_z_input = fftshift(fft(r_ofdm_symbol_wo_cp))*scale;
    r_z_input = r_z_input.';
    start1 = num_zeros1 + 1;
